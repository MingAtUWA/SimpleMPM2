...
res_tree = xml_etree.parse("..\\Build\\TestsWithGL\\fem_chm_uup_res_1d_conso.xml")
...
for th in root.findall("TimeHistory"):
    ...
    p.append(float(data_text[4]))
    #p.append(float(data_text[1]))
...
for i in range(data_num):
    ...
    u_list[i + 2] = con_res.calPorePressure(t_list[i + 2], 1.0)
    t_list[i + 2] += t_list[1]
