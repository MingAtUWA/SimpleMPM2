...
void test_t2d_mpm_chm_s_t_bar_real(void)
{
	...
	res_file_xml.init("t2d_mpm_chm_t_bar_real.xml");
...
void test_color_animation_t2d_chm_s_t_bar_real(void)
{
	...
	GA_T2D_CHM_s_color gen(1000, 1000); // window size
	gen.init_color_graph(-5.0e3, 5.0e3, colors, sizeof(colors) / sizeof(ColorGraph::Colori));
	gen.generate(5.0, -3.2, 3.2, -3.7, 0.5,
		"t2d_mpm_chm_t_bar_real.bin", "t2d_mpm_chm_t_bar_real.gif");
}