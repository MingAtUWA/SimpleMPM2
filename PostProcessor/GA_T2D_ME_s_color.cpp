...
int GA_T2D_ME_s_color::init(const char *res_file_name)
{
	...
	if (mph.pcl_num)
	{
		mp_x_data = new double[mph.pcl_num * 6];
		mp_y_data = mp_x_data + mph.pcl_num;
		mp_vol_data = mp_y_data + mph.pcl_num;
		mp_s11_data = mp_vol_data + mph.pcl_num;
		mp_s22_data = mp_s11_data + mph.pcl_num;
		mp_s12_data = mp_s22_data + mph.pcl_num;
	}
	...
	if (rch.pcl_num) // has rigid body
	{
		time_rcd_len = sizeof(TimeHistoryHeader)
					 ...
					 + sizeof(MPObjectHeader) + mph.pcl_num * 6 * sizeof(double);
	}
	else
	{
		time_rcd_len = sizeof(TimeHistoryHeader)
...
int GA_T2D_ME_s_color::render_frame(double xl, double xu, double yl, double yu)
{
	...
	if (rc_pcl_num)
		...
		res_file.seekg(first_time_rcd_file_pos + cur_time_rcd_id * time_rcd_len + sizeof(TimeHistoryHeader), SEEK_SET);
	res_file.read(reinterpret_cast<char *>(&mph), sizeof(mph));
	res_file.read(reinterpret_cast<char *>(mp_x_data), sizeof(double) * mph.pcl_num * 6);
	ColorGraph::Colorf pcl_color;
	pcls_mem.reset();
	for (size_t pcl_id = 0; pcl_id < mph.pcl_num; ++pcl_id)
	{
		pcl_color = color_graph.get_color(mp_s22_data[pcl_id]);
