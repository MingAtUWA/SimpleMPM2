...
public:
	TimeHistoryOutput_T2D_CHM_s_SE(const char *_name) : 
		...
		th_id(-1) {}
	~TimeHistoryOutput_T2D_CHM_s_SE() { close(); }
...
protected:
	hid_t th_id;
public:
	friend int time_history_output_func_t2d_chm_s_SE_to_hdf5_res_file(TimeHistoryOutput &_self);
	inline void set_res_file(ResultFile_hdf5 &_res_file) noexcept
	{
		res_file = &_res_file;
		output_func = &time_history_output_func_t2d_chm_s_SE_to_hdf5_res_file;
	}
};
