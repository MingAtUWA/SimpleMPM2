...
int solve_substep_S2D_CHM_s_GIMP(void *_self)
{
	...
	for (size_t pcl_id = 0; pcl_id < model.pcl_num; ++pcl_id)
	{
		...
		if (pcl.elem_num)
		{
			...
			double avg_de11, avg_de22, avg_de12;
			double avg_ds11, avg_ds22, avg_ds12;
...
int solve_substep_S2D_CHM_s_avg_stress_GIMP(void *_self)
{
	...
	for (size_t pcl_id = 0; pcl_id < model.pcl_num; ++pcl_id)
	{
		...
		{
			if (model.init_pcl_GIMP(pcl))
				pcl.var.pe->has_pcl = true;
		}
	}

	// Map mass, velocity and calculate drag force
	for (size_t pcl_id = 0; pcl_id < model.pcl_num; ++pcl_id)
	{
		Particle_mpm &pcl = model.pcls[pcl_id];
		double n_prod_miu_div_k = pcl.n * pcl.miu / pcl.k;
		for (size_t var_id = 0; var_id < pcl.elem_num; ++var_id)
		{
			ParticleCalVar_mpm &pcl_var = pcl.vars[var_id];
			Element_mpm &var_elem = *(pcl_var.pe);
			if (var_elem.has_pcl)
			{
				// map stress to element centre
				var_elem.pcl_vol += pcl_var.vol;
				var_elem.avg_s11 += pcl.s11 * pcl_var.vol;
				var_elem.avg_s22 += pcl.s22 * pcl_var.vol;
				var_elem.avg_s12 += pcl.s12 * pcl_var.vol;
				var_elem.avg_p += pcl.p * pcl_var.vol;
	...
	for (size_t tf_id = 0; tf_id < model.tx_num; ++tf_id)
	{
		...
		{
			ParticleCalVar_mpm &pcl_var = pcl.var;
			double pcl_vol = pcl.m_s / ((1.0 - pcl.n) * pcl.density_s);
			double t_tmp = sqrt(pcl_var.vol / pcl_vol) * tf.t;
			// node 1
			Node_mpm &n1 = *pcl_var.pn1;
			n1.fx_ext_m += pcl_var.N1 * t_tmp;
			// node 2
	...
	for (size_t pcl_id = 0; pcl_id < model.pcl_num; ++pcl_id)
	{
		...
		for (size_t var_id = 0; var_id < pcl.elem_num; ++var_id)
		{
			...
			if (pcl_var.pe->has_pcl)
			{
				...
				// porosity
				pcl_n = (pcl.n + de_vol_s) / (1.0 + de_vol_s);
				//dn = (1.0 - pcl.n) * de_vol_s / (1.0 + de_vol_s);
				avg_n += pcl_n * pcl_var.vol;
