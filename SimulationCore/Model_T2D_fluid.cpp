...
Model_T2D_fluid::Model_T2D_fluid() :
	Model("Model_T2D_fluid"),
...
void Model_T2D_fluid::init_mesh(
...
void Model_T2D_fluid::clear_mesh(void)
{
	if (nodes)
	{
		delete[] nodes;
...
void Model_T2D_fluid::init_pcls(
	size_t num,
	double m,
	double density,
	double _miu,
...
void Model_T2D_fluid::alloc_pcls(size_t num)
{
	pcl_num = num;
	pcls = new Particle[pcl_num];
}

void Model_T2D_fluid::clear_pcls(void)
{
	if (pcls)
	{
		delete[] pcls;
...
void Model_T2D_fluid::init_mesh(TriangleMesh &tri_mesh)
{
	clear_mesh();
	...
	)
{
	...
	if (pcl_num == 0)
		return;
	init_pcls(pcl_num, density, density, _miu, _lambda, _Kf);
...
int Model_T2D_fluid::init_bg_mesh(double hx, double hy)
{
	if (elem_num == 0)
		return -1;
...
void Model_T2D_fluid::add_elem_to_bg_grid(Element &e)
{
