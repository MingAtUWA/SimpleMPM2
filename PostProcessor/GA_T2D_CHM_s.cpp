...
GA_T2D_CHM_s::GA_T2D_CHM_s() : mp_x_data(nullptr) {}
...
int GA_T2D_CHM_s::init(const char *res_file_name)
{
...
int GA_T2D_CHM_s::render_frame(double xl, double xu, double yl, double yu)
{
	...
	for (size_t pcl_id = 0; pcl_id < mph.pcl_num; ++pcl_id)
		pcls_mem.add_pcl(mp_x_data[pcl_id], mp_y_data[pcl_id], mp_vol_data[pcl_id]*0.25);
	mp_data.clear();
