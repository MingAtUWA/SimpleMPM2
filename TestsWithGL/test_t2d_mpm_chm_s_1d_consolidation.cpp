...
void test_t2d_mpm_chm_s_1d_consolidation(void)
{
	...
	//GLfloat pt_coord;
	//Model_T2D_CHM_s::Particle &pcl = model.pcls[17];
	////Model_T2D_CHM_s::Node &n = model.nodes[26];
	//pt_array.reserve(3);
	//pt_coord = double(pcl.x);
	//pt_array.add(&pt_coord);
	//pt_coord = double(pcl.y);
	...
	//step.set_bv_ratio(0.01); // bulk viscosity
	step.set_time(15.0); // 0.92 for p = 100
	...
	//Step_T2D_CHM_s step2; // explode at 0.92s
